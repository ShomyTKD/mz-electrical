...
local function DrawPackageLocationBlip()
    DrawMarker(2, BreakdownLocations.x, BreakdownLocations.y, BreakdownLocations.z + 3, 0, 0, 0, 180.0, 0, 0, 0.5, 0.5,
        0.5, 255, 255, 0, 100, false, false, 2, true, nil, nil, false)
end
local function DrawPackageLocationBlipTier2()
    DrawMarker(2, BreakdownLocationsTier2.x, BreakdownLocationsTier2.y, BreakdownLocationsTier2.z + 3, 0, 0, 0, 180.0, 0,
        0, 0.5, 0.5, 0.5, 255, 255, 0, 100, false, false, 2, true, nil, nil, false)
end
local function DrawPackageLocationBlipTier3()
    DrawMarker(2, BreakdownLocationsTier3.x, BreakdownLocationsTier3.y, BreakdownLocationsTier3.z + 3, 0, 0, 0, 180.0, 0,
        0, 0.5, 0.5, 0.5, 255, 255, 0, 100, false, false, 2, true, nil, nil, false)
end
local function DrawPackageLocationBlipTierPrep()
    DrawMarker(2, BreakdownLocationsTierPrep.x, BreakdownLocationsTierPrep.y, BreakdownLocationsTierPrep.z + 4, 0, 0, 0,
        180.0, 0, 0, 0.5, 0.5, 0.5, 0, 255, 0, 100, false, false, 3, true, nil, nil, false)
end
...
CreateThread(function()
    local model = `s_f_y_airhostess_01`
    RequestModel(model)
    while not HasModelLoaded(model) do
        Wait(0)
    end
    local entity = CreatePed(0, model, 727.8606, 132.3552, 79.9564, 59.1075, true, false)
    ...
    exports['qb-target']:AddTargetEntity(entity, { -- The specified entity number
        options = {
            ...
            },
            {
                ...
                label = 'Collect Your Payment'
            },
            {
                type = "client",
                event = "mz-electrical:client:EndWork",
                icon = 'fas fa-tools',
                label = 'End Shift Earlier (No Payment)'
            },
        },
        distance = 1.5,
    })
...
RegisterNetEvent('mz-electrical:client:ToggleDuty', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.name == "technician" then
            ...
            if Config.NotifyType == 'qb' then
                QBCore.Functions.Notify("You aren't a technician... Please visit the Cityhall/Job centre...", "primary",
                    3000)
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("NOT A TECH?",
                    "You aren't a technician... Please visit the Cityhall/Job centre...", 3000, "info")
            end
        end
    end)
end)
...
RegisterNetEvent('mz-electrical:client:Tutorial', function()
    if not TutorialStarted then
        ...
        Wait(4250)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("The city generators are constantly under high stress and need your maintenance",
                "primary", 4000)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "The city generators are constantly under high stress and need your maintenance", 4000, "info")
        end
        Wait(4250)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify(
                "In some cities, you will need to apply for 'Electrical Technician' employment before you can work here.",
                "primary", 5000)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "In some cities, you will need to apply for 'Electrical Technician' employment before you can work here.",
        ...
        Wait(5250)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("You can usually do that by visiting City Hall or your city's Job Centre.", "primary",
                4000)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "You can usually do that by visiting City Hall or your city's Job Centre.", 4000, "info")
        end
        Wait(4250)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify(
                "Once you have the required job, you may need to gain some Electrical XP to help out.", "primary", 4500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "Once you have the required job, you may need to gain some Electrical XP to help out.", 4500, "info")
        end
        Wait(4750)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify(
                "Not all cities have Electrical XP. If yours does not then you can do all work from the start!",
                "primary",
                4500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "Not all cities have Electrical XP. If yours does not then you can do all work from the start!", 4500,
                "info")
        end
        Wait(4750)
        if Config.NotifyType == 'qb' then
        ...
        Wait(2500)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("Just head over from where you are to the tool location on your right.", "primary",
                3500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "Just head over from where you are to the tool location on your right.", 3500, "info")
        end
        Wait(4500)
        ...
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify(
                "When you are finished, return your supplies here to get paid (and start a new job) at this same place.",
                "primary", 5500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "When you are finished, return your supplies here to get paid (and start a new job) at this same place.",
        ...
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME", "Once you have a bit of experience, you can do some more work.", 3500,
                "info")
        end
        Wait(3750)
        if Config.NotifyType == 'qb' then
        ...
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME", "Over on the other side of the plant you can start Tier 2 jobs.", 4500,
                "info")
        end
        Wait(1000)
        ...
        Wait(5000)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("And back next to the Tier 1 jobs you can start the Tier 3 jobs here.", "primary",
                4500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME", "And back next to the Tier 1 jobs you can start the Tier 3 jobs here.",
                4500, "info")
        end
        Wait(1000)
        ...
        Wait(5000)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("When you are done with any T1, T2 or T3 job, you can get paid here.", "primary",
                3500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME", "When you are done with any T1, T2 or T3 job, you can get paid here.",
                3500, "info")
        end
        Wait(1000)
        ...
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME", "You cannot start more than one job before collecting your payment.",
                3500, "info")
        end
        Wait(3750)
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("Look for the double doors near the Prep Tier work, closest to the entrance.",
                "primary", 4500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "Look for the double doors near the Prep Tier work, closest to the entrance.", 4500, "info")
        end
        Wait(1000)
        ...
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify(
                "You can go through this tutorial as many times as you like, please visit again if you need to.",
                "primary",
                4500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("WELCOME",
                "You can go through this tutorial as many times as you like, please visit again if you need to.", 4500,
                "info")
        end
        Wait(4500)
        if Config.NotifyType == 'qb' then
        ...
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("You cannot skip the tutorial or start it again, please pay attention.", "error",
                3500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("FOREMAN",
                "You cannot skip the tutorial or start it again, please pay attention.", 3500, "error")
        end
    end
end)
    ...
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if Config.NeedJob then
            if PlayerData.job.name == "technician" then
                        ...
                        if hasskill then
                            ...
                            if Config.NotifyType == 'qb' then
                                QBCore.Functions.Notify(
                                    "You need at least " ..
                                    Config.tierPrepXP ..
                                    " " .. Config.ElecXPskill .. " XP to attend to this sort of work.",
                                    "error", 3500)
                            elseif Config.NotifyType == "okok" then
                                exports['okokNotify']:Alert("MORE XP NEEDED",
                                    "You need at least " ..
                                    Config.tierPrepXP .. " " .. Config.ElecXPskill ..
                                    " XP to attend to this sort of work.", 3500, "error")
                            end
                        end
                    end)
                else
                ...
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify(
                        "You are not qualified to assist here. Please go to the job centre and apply for technician employment.",
                        "primary", 3500)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("NOT QUALIFIED?",
                        "You are not qualified to assist here. Please go to the job centre and apply for technician employment.",
                        3500, "info")
                end
            end
                ...
                exports["mz-skills"]:CheckSkill(Config.ElecXPskill, Config.tierPrepXP, function(hasskill)
                    if hasskill then
                        ...
                        if Config.NotifyType == 'qb' then
                            QBCore.Functions.Notify(
                                "You need at least " ..
                                Config.tierPrepXP .. " " .. Config.ElecXPskill .. " XP to attend to this sort of work.",
                                "error", 3500)
                        elseif Config.NotifyType == "okok" then
                            exports['okokNotify']:Alert("MORE XP NEEDED",
                                "You need at least " ..
    ...
    if not jobTier1 and not jobTier2 and not jobTier3 then
        if not jobStart then
            if not jobFinished then
                ...
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify(
                        "As an apprentice it is important to make sure the worksite is properly prepared.", "primary",
                        3000)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("PREPARATION",
                        "As an apprentice it is important to make sure the worksite is properly prepared.", 3000, "info")
                end
                Wait(3000)
                TriggerEvent('animations:client:EmoteCommandStart', { "mechanic" })
                Wait(500)
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify("This involves ensuring that equipment is well maintained and clean.",
                        "primary", 3000)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("WORK",
                        "This involves ensuring that equipment is well maintained and clean.", 3000, "info")
                end
                Wait(1000)
                ...
                QBCore.Functions.Progressbar("search_register", "Checking requirements...", Config.GetJob, false, true, {
                    ...
                    if Config.NotifyType == 'qb' then
                        QBCore.Functions.Notify("Please move to the blip to attempt to resolve the issue.", "primary",
                            3000)
                    elseif Config.NotifyType == "okok" then
                        exports['okokNotify']:Alert("MOVE TO PING",
                            "Please move to the blip to attempt to resolve the issue.", 3000, "info")
                    end
                    Wait(500)
                ...
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("WORK COMPLETE", "You are done, please return your supplies.", 3500,
                        "info")
                end
            end
        else
            ...
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("WORK STARTED", "You have already started work. Please check for the blip...",
                    3500, "info")
            end
            Wait(1000)
            if Config.NotifyType == 'qb' then
        ...
        if Config.NotifyType == 'qb' then
            QBCore.Functions.Notify("You are already consigned to do something else, please complete that work...",
                "primary", 3500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("OTHER JOB?",
                "You are already consigned to do something else, please complete that work...", 3500, "info")
        end
    end
end)
...
RegisterNetEvent('mz-electrical:client:PrepareWorksite', function()
    if not working then
        ...
        TriggerEvent('animations:client:EmoteCommandStart', { "mechanic" })
        Wait(1000)
        QBCore.Functions.Progressbar("search_register", "Preparing work station...", Config.PrepareWorkStation, false,
            true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
...
RegisterNetEvent('mz-electrical:client:PrepPhase1', function()
    ...
    QBCore.Functions.Progressbar("search_register", "Cleaning up site...", Config.CleanUp, false, true, {
        ...
        if jobsCompletePrep == Config.TierPrepJobs then
            Wait(2000)
            if Config.NotifyType == 'qb' then
                QBCore.Functions.Notify("You have finished your job for the day, please return your supplies.", "success",
                    3500)
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("WORK COMPLETE",
                    "You have finished your job for the day, please return your supplies.", 3500, "success")
            end
            ...
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("NEXT SITE", "Please move to the next location when you are ready.", 3500,
                    "info")
            end
            GetRandomWorkTierPrep()
            Wait(500)
...
CreateThread(function()
    ...
    local entity = CreatePed(0, model, 718.6811, 152.4266, 79.7545, 150.1505, true, false)
                    ...
                    exports["mz-skills"]:CheckSkill(Config.ElecXPskill, Config.tier1XP, function(hasskill)
                        if hasskill then
                            ...
                            if Config.NotifyType == 'qb' then
                                QBCore.Functions.Notify(
                                    "You need at least " ..
                                    Config.tier1XP .. " " .. Config.ElecXPskill .. " XP to attend to this sort of work.",
                                    "error", 3500)
                            elseif Config.NotifyType == "okok" then
                                exports['okokNotify']:Alert("MORE XP NEEDED",
                                    "You need at least " ..
    ...
    if not jobTier2 and not jobTier3 then
        if not jobStart then
            if not jobFinished then
                ...
                Wait(2000)
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify("FOREMAN: Yes yes, just across from us. Please get them functioning again!",
                        "neutral", 3000)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("FOREMAN",
                        "Yes yes, just across from us. Please get them functioning again!", 3000, "neutral")
                end
                Wait(1000)
                TriggerEvent('animations:client:EmoteCommandStart', { "clipboard" })
                ...
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify("You have already finished repairing the generator, collect your payment.",
                        "primary", 3500)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("WORK COMPLETE",
                        "You have already finished repairing the generator, collect your payment.", 3500, "info")
                end
            end
        else
            if Config.NotifyType == 'qb' then
                QBCore.Functions.Notify("You have already started work. Please find the fault at the blip...", "primary",
                    3500)
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("WORK STARTED",
                    "You have already started work. Please find the fault at the blip...", 3500, "info")
            end
            Wait(1000)
            if Config.NotifyType == 'qb' then
                QBCore.Functions.Notify("Cooldown remaining: " .. jobtime .. " seconds.", "error", 3500)
...
CreateThread(function()
    ...
    local entity = CreatePed(0, model, 669.7056, 100.9533, 79.7546, 6.3900, true, false)
                    ...
                    exports["mz-skills"]:CheckSkill(Config.ElecXPskill, Config.tier2XP, function(hasskill)
                        if hasskill then
                            ...
                            if Config.NotifyType == 'qb' then
                                QBCore.Functions.Notify(
                                    "You need at least " ..
                                    Config.tier2XP .. " " .. Config.ElecXPskill .. " XP to attend to this sort of work.",
                                    "error", 3500)
                            elseif Config.NotifyType == "okok" then
                                exports['okokNotify']:Alert("MORE XP NEEDED",
                                    "You need at least " ..
    ...
    if not jobTier1 and not jobTier3 then
        if not jobStart then
            if not jobFinished then
                ...
                Wait(2000)
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify("FOREMAN: Ahh you are finally here, yes yes, have a look here...", "neutral",
                        3000)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("FOREMAN", "Ahh you are finally here, yes yes, have a look here...", 3000,
                        "neutral")
                end
                Wait(500)
                TriggerEvent('animations:client:EmoteCommandStart', { "clipboard" })
...
CreateThread(function()
    local model = `u_m_m_promourn_01`
    RequestModel(model)
    while not HasModelLoaded(model) do
        Wait(0)
    end
    local entity = CreatePed(0, model, 711.7904, 164.8896, 79.7545, 159.8230, true, false)
                    ...
                    exports["mz-skills"]:CheckSkill(Config.ElecXPskill, Config.tier3XP, function(hasskill)
                        if hasskill then
                            ...
                            if Config.NotifyType == 'qb' then
                                QBCore.Functions.Notify(
                                    "You need at least " ..
                                    Config.tier3XP .. " " .. Config.ElecXPskill .. " XP to attend to this sort of work.",
                                    "error", 3500)
                            elseif Config.NotifyType == "okok" then
                                exports['okokNotify']:Alert("MORE XP NEEDED",
                                    "You need at least " ..
    ...
    if not jobTier1 and not jobTier2 then
        if not jobStart then
            if not jobFinished then
                ...
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("YOU", "They've sent the most experienced sparky they have...", 3000,
                        "info")
                end
                Wait(2000)
                if Config.NotifyType == 'qb' then
                    QBCore.Functions.Notify("FOREMAN: Excellent, please fix this, the city needs the power!", "neutral",
                        3000)
                elseif Config.NotifyType == "okok" then
                    exports['okokNotify']:Alert("FOREMAN", "Excellent, please fix this, the city needs the power!", 3000,
                        "neutral")
                end
                Wait(500)
                TriggerEvent('animations:client:EmoteCommandStart', { "clipboard" })
            ...
            exports['ps-ui']:Circle(function(success)
                if success then
                    ...
                    if Config.ElectrocutionChance >= zapchance then
                        ...
                        if Config.NotifyType == 'qb' then
                            QBCore.Functions.Notify(
                                "You fail to fix the wires and almost electructe yourself... Be careful!", "error", 3500)
                        elseif Config.NotifyType == "okok" then
                            exports['okokNotify']:Alert("WIRES BROKEN",
                                "You fail to fix the wires and almost electructe yourself... Be careful!", 3500, "error")
                        end
                        TriggerEvent('animations:client:EmoteCommandStart', { "c" })
                    end
                    working = false
...
RegisterNetEvent('mz-electrical:client:FixWiresProgressbar', function()
    QBCore.Functions.Progressbar("search_register", "Fixing loose wires...", Config.FixWires, false, true, {
        ...
        if jobsComplete == Config.Tier1Jobs or jobsCompleteT2 == Config.Tier2Jobs or jobsCompleteT3 == Config.Tier3Jobs then
            Wait(2000)
            if Config.NotifyType == 'qb' then
                QBCore.Functions.Notify("This generator is back to functioning, head over to head office!", "success",
                    3500)
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("GENERATOR REPAIRED",
                    "This generator is back to functioning, head over to head office!", 3500, "success")
            end
            jobFinished = true
            BreakdownLocations = nil
            BreakdownLocationsTier2 = nil
            ...
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("NEW FAULT", "Please move to the next fault when you are ready.", 3500,
                    "info")
            end
            if jobTier1 then
                GetRandomWork()
